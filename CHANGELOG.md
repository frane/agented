...
`open`, `load` and every write verb already reconciled with disk. `view`, `search`, `diff` and `find` did not: they read the stored head, with no drift detection and no warning, which made them the cheapest read path and the only lying one. They now reconcile on the same terms as `open`:

- Disk diverged from head → the disk content is folded in as a new `load` edit (recoverable via `ae undo` / `ae branches`), the read is answered from disk, and a `warning:` line names the new edit. Drift-free reads stat-fast-path and create no edits.
- `concurrency.read_drift` selects the behavior: `reconcile` (default, above), `warn` (serve the workspace head, print a `STALE:` warning naming path and head edit, set `stale: true` in JSON/MCP, **exit 3** — so `ae view … && …` stops instead of acting on content that does not match disk), or `refuse` (error, serve nothing, exit 3).
- `find` reconciles each file it searches and reports the files it could not, so a cross-file search can no longer answer "not present anywhere" out of stale buffers.

**A file deleted from disk is reported, not resurrected.** `ae open` on a path that ae has registered but that no longer exists on disk used to recreate it; a deliberately removed 716-line component came back that way and broke the reporting repo's orphan-component test. Deletion is now a first-class answer: `open`, the content reads, and `save` all fail with `file deleted on disk` (exit 1) and name the two ways out — `ae save --force <path>` to restore from head, `ae close <path>` to drop the path. Only genuinely new paths are still auto-created by `open`. `find` skips vanished files and lists them in its warning.

**`ae rollback` now mirrors onto disk.** Per-op autosaves during a transaction leave the rolled-back content on disk; rollback only moved head pointers, so disk kept the reverted edits. Undo and redo already reflushed — explicit and idle auto-rollback now do too. This was invisible while reads came from the store; with reads reconciling against disk it would have un-rolled-back the transaction on the next read.

**Concurrent writes no longer fail with SQLITE_BUSY.** Found while answering a follow-up about what reconcile-on-read does under a fleet, and the more serious of the two: `withWriteTx` used `database/sql`'s `Begin`, which issues a *deferred* `BEGIN`. Nearly every transaction here reads before it writes (read head, insert edit), so it starts on a read snapshot and only asks for the write lock later; when another process has committed in between, SQLite fails that upgrade immediately with `SQLITE_BUSY_SNAPSHOT` and `busy_timeout` deliberately does not wait, because waiting could never help. Measured: six concurrent readers of one drifted file, **five failed** with `database is locked (5) (SQLITE_BUSY)`. The transaction is now retried from a fresh snapshot with jittered backoff; the same six all succeed, all get the on-disk content, and they produce exactly one `load` edit between them. `withWriteTx` documents that its closure may run more than once, and the three closures that accumulated state (`Prune`'s report, `AutoRollbackIdle`'s slice) reset per attempt. This affected every concurrent write, not just the new read path — reads simply made it reachable.

**Read verbs return content and state token from one snapshot.** `HeadContent` resolves the head and reconstructs it in two independent reads, so a verb pairing it with a separately-fetched `FileInfo` could hand back a token describing a different head than the bytes beside it. New `HeadContentWithInfo` does both under one read transaction; `view` and `search` use it.

**`concurrency.read_drift` replaces the drift bool for reads**, on the reporter's suggestion that reconcile and refuse aren't the only two things someone might want: `reconcile` (default), `warn` (serve head, flag stale, exit 3), `refuse` (error, serve nothing, exit 3). `auto_load_on_drift=false` still forces at least `warn`, so existing configs keep the behavior they asked for. Env: `AE_READ_DRIFT`.
