...
- **Regex replace with capture groups.** `ae replace --pattern` does sed-style replacement in a single tool call. Verb: `ae s <path> -p '<re>' -w '<expansion>' [-L <max>] [-n]`. **Always single-quote `-w`** when using `$1`/`$2` backrefs — bash expands `$1` as the first positional arg (empty in most shells), so `-w "$1.foo"` silently inserts an empty string. Single quotes (`-w '$1.foo'`) pass `$1` through to ae, which expands per Go regexp.ExpandString semantics. **A reference to a group the pattern doesn't have is an error, not an empty string** — Go would silently expand `${id}` to nothing, which is how a TypeScript template literal becomes `` `${xs.map(id => ``)}` ``: still valid syntax, so the compiler accepts it. For replacement text containing `$` that is not a backref (shell vars, JS template literals), pass `-l`/`--literal` to insert `-w` verbatim with no expansion, or escape each dollar as `$$`. Note there are **two** independent ways a `$` gets eaten and quoting only fixes one: double quotes let the shell expand `${id}` before ae sees it, and single quotes stop that but ae's own expansion still resolves an unknown group. `--literal` is the only thing that handles both. **0 matches is an error** (nonzero exit, nothing written) so `&& ae save` chains stop instead of shipping a no-op; pass `--allow-no-match` when 0 is an acceptable outcome. `--text` is accepted as an alias for `--with`.
...
**Auto-save and auto-load are on by default.** Every write verb (replace/insert/delete/move/extract) and history verb (undo/redo/head) flushes the resulting head to disk in the same call. The result includes `saved: true` to confirm. Before the write, ae stat-s the file: if `(mtime, size)` match the stamp from our last save, the call proceeds; if disk was touched externally, ae loads the disk content as a new edit on the tree (so external changes are recoverable via `ae undo`/`ae head`) and applies your edit on top. The result includes `loaded_from_disk: true` and `drift_reason` when this happens. `ae open` performs the same reconciliation: opening a file whose disk content diverged from the workspace head folds the disk state in as a new head edit and says so. **The content read verbs (`view`, `search`, `diff`, `find`) reconcile too** — they never answer from a workspace head that disk has moved past. When drift is found the content served is disk's, and a `warning:` line names the new edit. `concurrency.read_drift` picks what a read does about drift: `reconcile` (default, as above), `warn` (serve the workspace head, print a `STALE:` warning, set `stale: true`, exit 3, so a `&&` chain stops rather than acting on it), or `refuse` (error, serve nothing, exit 3). The older `auto_load_on_drift=false` still forces at least `warn`. Reconciling means a read can create an edit; the fold is serialised, so N agents reading the same drifted path produce one `load` edit and all are served the same content. A path registered in the workspace but **deleted from disk** is an error (`file deleted on disk`, exit 1) on read, `open` and `save` — ae will not serve, recreate, or restore a file somebody removed. `ae save --force <path>` restores it from head on purpose; `ae close <path>` drops it from the workspace.
